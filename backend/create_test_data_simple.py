...
def create_test_data():
    ...
    try:
        ...
        # 練習種別作成
        workout_types_data = [
            {"name": "easy_run", "display_name": "イージーラン"},
            {"name": "tempo_run", "display_name": "テンポ走"},
            {"name": "interval", "display_name": "インターバル走"},
            {"name": "long_run", "display_name": "ロング走"},
            {"name": "recovery", "display_name": "回復走"},
        ]
        ...
        for wt_data in workout_types_data:
            ...
            if not workout_type:
                workout_type = WorkoutType(
                    ...
                    description=f"{wt_data['display_name']}の練習",
                    is_default=True
                )
                db.add(workout_type)
                db.commit()
        ...
        while current_date <= date.today():
            # 週間パターンでデータを作成
            weekly_pattern = [
                ("easy_run", 2, 5000, [1200, 1200, 1200, 1200, 200]),
                ("interval", 4, 4000, [400, 400, 400, 400, 400, 400, 400, 400, 400, 400]),
                ("recovery", 1, 3000, [1800]),
                ("tempo_run", 3, 6000, [1800, 1800, 1800]),
                None,  # 休息日
                ("long_run", 2, 15000, [4500, 4500, 4500]),
                ("easy_run", 2, 4000, [1200, 1200, 1200, 400]),
            ]
            ...
            for day_offset, workout_info in enumerate(weekly_pattern):
                ...
                workout = Workout(
                    ...
                    notes=f"{workout_types[workout_type_name].display_name} - テストデータ"
                )
